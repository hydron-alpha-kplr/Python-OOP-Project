...
def create_tree_from_dict(json_dict):
    ...
    recursive_tree_from_json(json_dict, "racine", my_tree)

    #Retourner l'arbre
    return my_tree

# Fonction récursive pour parcourir un dictionnaire Python et créer des noeuds dans un arbre
def recursive_tree_from_json(json_dict, parent_node_id, tree):
    ...
    for key, value in json_dict.items():
        ...
        if "subclasses" in value :
            recursive_tree_from_json(value["subclasses"], new_node_id, tree)
...
def main():
    ...
    tree = create_tree_from_dict(json_dict)
    tree.show()
